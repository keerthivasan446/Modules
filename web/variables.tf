...
variable "primary_extra_tags" { type = map(any) }
variable "secondary_extra_tags" { type = map(any) }
variable "tertiary_extra_tags" { type = map(any) }
...
variable "instance_ingress_sg_proto" { type = list(any) }
